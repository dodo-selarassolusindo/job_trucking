...
class DepoEdit extends Depo
{
    ...
    // CSS class/style
    public $CurrentPageName = "depoedit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("DepoID") ?? Key(0) ?? Route(2)) !== null) {
                $this->DepoID->setQueryStringValue($keyValue);
                $this->DepoID->setOldValue($this->DepoID->QueryStringValue);
            } elseif (Post("DepoID") !== null) {
                $this->DepoID->setFormValue(Post("DepoID"));
                $this->DepoID->setOldValue($this->DepoID->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("DepoID") ?? Route("DepoID")) !== null) {
                    $this->DepoID->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->DepoID->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("depolist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "depolist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "depolist") {
                            ...
                            $returnUrl = "depolist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("depolist"), "", $this->TableVar, true);
        $pageId = "edit";
