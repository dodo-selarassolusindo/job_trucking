 ...
 * Page class
 */
class Size22Edit extends Size22
{
    use MessagesTrait;
    ...
    // CSS class/style
    public $CurrentPageName = "size22edit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("SizeID") ?? Key(0) ?? Route(2)) !== null) {
                $this->SizeID->setQueryStringValue($keyValue);
                $this->SizeID->setOldValue($this->SizeID->QueryStringValue);
            } elseif (Post("SizeID") !== null) {
                $this->SizeID->setFormValue(Post("SizeID"));
                $this->SizeID->setOldValue($this->SizeID->FormValue);
            } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("size22list"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "size22list") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "size22list") {
                            ...
                            $returnUrl = "size22list"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function restoreEditFormFromRow($row)
    {
        if (isset($row['Ukuran'])) { // Ukuran
            $this->Ukuran->CurrentValue = $row['Ukuran'];
        }
    }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("size22list"), "", $this->TableVar, true);
        $pageId = "edit";
