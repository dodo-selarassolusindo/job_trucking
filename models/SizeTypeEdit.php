 ...
 * Page class
 */
class SizeTypeEdit extends SizeType
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SizeTypeEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "sizetypeedit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("Size_Type_ID") ?? Key(0) ?? Route(2)) !== null) {
                $this->Size_Type_ID->setQueryStringValue($keyValue);
                $this->Size_Type_ID->setOldValue($this->Size_Type_ID->QueryStringValue);
            } elseif (Post("Size_Type_ID") !== null) {
                $this->Size_Type_ID->setFormValue(Post("Size_Type_ID"));
                $this->Size_Type_ID->setOldValue($this->Size_Type_ID->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("Size_Type_ID") ?? Route("Size_Type_ID")) !== null) {
                    $this->Size_Type_ID->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->Size_Type_ID->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("sizetypelist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "sizetypelist") {
                            ...
                            $returnUrl = "sizetypelist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'Size_Type_ID' first before field var 'x_Size_Type_ID'
        $val = $CurrentForm->hasValue("Size_Type_ID") ? $CurrentForm->getValue("Size_Type_ID") : $CurrentForm->getValue("x_Size_Type_ID");
        if (!$this->Size_Type_ID->IsDetailKey) {
            $this->Size_Type_ID->setFormValue($val);
        }
    ...
    public function renderRow()
    {
        ...
        // Size_Type_ID
        $this->Size_Type_ID->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->Size_Type_ID->Visible && $this->Size_Type_ID->Required) {
                if (!$this->Size_Type_ID->IsDetailKey && EmptyValue($this->Size_Type_ID->FormValue)) {
                    $this->Size_Type_ID->addErrorMessage(str_replace("%s", $this->Size_Type_ID->caption(), $this->Size_Type_ID->RequiredErrorMessage));
                }
            }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("sizetypelist"), "", $this->TableVar, true);
        $pageId = "edit";
