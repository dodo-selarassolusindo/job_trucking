 ...
 * Page class
 */
class TakenByEdit extends TakenBy
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "TakenByEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "takenbyedit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("TakenByID") ?? Key(0) ?? Route(2)) !== null) {
                $this->TakenByID->setQueryStringValue($keyValue);
                $this->TakenByID->setOldValue($this->TakenByID->QueryStringValue);
            } elseif (Post("TakenByID") !== null) {
                $this->TakenByID->setFormValue(Post("TakenByID"));
                $this->TakenByID->setOldValue($this->TakenByID->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("TakenByID") ?? Route("TakenByID")) !== null) {
                    ...
                    $this->TakenByID->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("takenbylist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "takenbylist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "takenbylist") {
                            ...
                            $returnUrl = "takenbylist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
