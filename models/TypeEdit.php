...
class TypeEdit extends Type
{
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("TypeID") ?? Key(0) ?? Route(2)) !== null) {
                $this->TypeID->setQueryStringValue($keyValue);
                $this->TypeID->setOldValue($this->TypeID->QueryStringValue);
            } elseif (Post("TypeID") !== null) {
                $this->TypeID->setFormValue(Post("TypeID"));
                $this->TypeID->setOldValue($this->TypeID->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("TypeID") ?? Route("TypeID")) !== null) {
                    $this->TypeID->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
