...
<span id="el_job_order_IsShow">
    <select
        ...
        data-select2-id="fjob_orderedit_x_IsShow"
        <?php } ?>
        data-table="job_order"
        data-field="x_IsShow"
...
<script>
loadjs.ready("fjob_orderedit", function() {
    var options = { name: "x_IsShow", selectId: "fjob_orderedit_x_IsShow" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fjob_orderedit.lists.IsShow?.lookupOptions.length) {
        options.data = { id: "x_IsShow", form: "fjob_orderedit" };
    } else {
        options.ajax = { id: "x_IsShow", form: "fjob_orderedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_job_order_IsOpen">
    <select
        ...
        data-select2-id="fjob_orderedit_x_IsOpen"
        <?php } ?>
        data-table="job_order"
        data-field="x_IsOpen"
...
<script>
loadjs.ready("fjob_orderedit", function() {
    var options = { name: "x_IsOpen", selectId: "fjob_orderedit_x_IsOpen" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fjob_orderedit.lists.IsOpen?.lookupOptions.length) {
        options.data = { id: "x_IsOpen", form: "fjob_orderedit" };
    } else {
        options.ajax = { id: "x_IsOpen", form: "fjob_orderedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
