...
?>
<main class="view">
<form name="fsize2view" id="fsize2view" class="ew-form ew-view-form overlay-wrapper" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<?php if (!$Page->isExport()) { ?>
<script>
...
<?php if ($Page->SizeID->Visible) { // SizeID ?>
    <tr id="r_SizeID"<?= $Page->SizeID->rowAttributes() ?>>
        <td class="<?= $Page->TableLeftColumnClass ?>"><span id="elh_size2_SizeID"><?= $Page->SizeID->caption() ?></span></td>
...
<?php if ($Page->Ukuran->Visible) { // Ukuran ?>
    <tr id="r_Ukuran"<?= $Page->Ukuran->rowAttributes() ?>>
        <td class="<?= $Page->TableLeftColumnClass ?>"><span id="elh_size2_Ukuran"><?= $Page->Ukuran->caption() ?></span></td>
