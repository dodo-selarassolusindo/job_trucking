...
<?php foreach($r as $row) { ?>
    ...
    ?>
        <div class="small-box <?= $rec_num % 2 == 0 ? 'bg-info' : 'bg-success' ?>">
            ...
            <a href="joborder2?job2id=<?= $_GET['job2id'] ?>&job2nama=<?= $_GET['job'] ?>&sizeid=<?= $_GET['sizeid'] ?>&sizeukuran=<?= htmlspecialchars($_GET['ukuran'], ENT_QUOTES) ?>&typeid=<?= $row['TypeID'] ?>&typenama=<?= $row['Nama'] ?>" class="small-box-footer">Lanjut <i class="fas fa-arrow-circle-right"></i></a>
        </div>
    <?php } ?>
    <?php $rec_num++ ?>
